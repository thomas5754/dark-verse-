  ...
  Future<void> _fetchSenders() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            "https://concerns-barn-roman-forth.trycloudflare.com/mySender?key=${widget.sessionKey}"),
  ...
  Future<void> _addSender(String number) async {
    ...
    try {
      final response = await http.get(Uri.parse(
          "https://concerns-barn-roman-forth.trycloudflare.com/getPairing?key=${widget.sessionKey}&number=$number"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data["valid"] == true) {
    ...
    if (confirmed == true) {
      ...
      try {
        final response = await http.delete(Uri.parse(
            "https://concerns-barn-roman-forth.trycloudflare.com/deleteSender?key=${widget.sessionKey}&id=$senderId"));
        if (response.statusCode == 200) {
          final data = jsonDecode(response.body);
          if (data["valid"] == true) {
